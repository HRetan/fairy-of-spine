...
**맥과 윈도우 실행파일**로 만들어두면 터미널도 Node 도 없이 더블클릭 한 번이면 돼. ✨

- 🎭 잔소리 문구 **30종** + 아스키 아트 **15종**을 돌려가며 써. 같은 말만 하면 사흘이면 무시하잖아 (4번 중 1번쯤 그림이 딸려 가)
- 🖥️ **설정 화면**이 있어. 토큰도 알림 규칙도 브라우저에서 넣으면 돼
- ⏱️ 알림 간격 / 활동 시간대 / 요일 / 타임존을 **채팅에서도** 바꿀 수 있어
- 🔘 알림에 `✅ 폈어요` `😴 15분 뒤` 버튼을 붙여줄게. 손가락 한 번이면 돼
- 📊 네가 허리 편 횟수를 날짜별로 세어둘게 (`/stats`)
- 📦 **단일 실행파일**로 뽑을 수 있어. 받는 사람은 아무것도 안 깔아도 돼
- 🪶 런타임 의존성은 하나도 없어. Node 24+ 가 TypeScript를 직접 읽으니 빌드도 필요 없어

## 🧺 준비물

- 텔레그램 봇 토큰, 디스코드 봇 토큰 중 **최소 하나**. 둘 다 줘도 좋아
- Node.js 24 이상 (`node -v`) — **실행파일을 만들 때만** 필요해.
  다 만든 뒤에는 그 실행파일을 쓰는 사람에겐 필요 없어
...
두 갈래가 있어. **터미널을 안 쓰고 싶으면 길 1** 로 가.

### 📦 길 1 — 실행파일 (터미널도 Node 도 필요 없어)

먼저 실행파일을 뽑아. 이건 한 번만 하면 돼.

```bash
npm install
npm run build:exe:all    # 맥용 + 윈도우용
```
...
| | 무엇 |
|---|---|
| `척추의 요정.app` | **맥 — 창 없이** 더블클릭 |
| `fairy-of-spine` | 맥 — 터미널에서 보며 쓸 때 |
| `조용히 실행.vbs` | **윈도우 — 콘솔 창 없이** 더블클릭 |
...
```bash
mkdir -p ~/Applications/척추의요정
cp -R "dist/척추의 요정.app" ~/Applications/척추의요정/
```

윈도우라면 `fairy-of-spine.exe` 와 `조용히 실행.vbs` 를 **둘 다** 같은 폴더에.
vbs 가 옆에 있는 exe 를 찾거든.

**2. 실행**

맥은 `척추의 요정.app`, 윈도우는 `조용히 실행.vbs` 를 더블클릭.
창은 안 떠. 그게 정상이야. 대신 **브라우저가 알아서 열려.**

안 열리면 직접 가도 돼 → `http://127.0.0.1:7979`

**3. 토큰 넣기**

설정 화면 아래 **📮 어디로 갈지** 에 텔레그램 봇 토큰을 넣고 `저장`.
토큰이 없으면 [@BotFather](https://t.me/BotFather) 에게 `/newbot` 이라고 하면 줘.

**4. 다시 켜기**

토큰은 시작할 때 한 번만 읽어. `🚪 나 끄기` 를 누르고 다시 더블클릭해줘.

**5. 어느 대화로 갈지 정하기**

설정 화면 맨 위 **🚀 시작하기** 에서 `텔레그램에서 열기` → 텔레그램에서 **시작** 누르기.
이 칸이 안 보이면 이미 정해진 거야. 📍

**6. 확인**

`지금 한 번 보내보기` 를 눌러봐. 텔레그램에 오면 끝. ✨

**짐은 전부 실행파일 옆에 쌓여.** 폴더째 옮기면 설정도 따라가.

```
어디든/
  척추의 요정.app  또는  fairy-of-spine.exe
  .env                     토큰
...
> ⚠️ 알아둘 것 셋
> - **맥 `.app` 은 Dock 에 안 떠.** 배경 앱이라 그래. 돌고 있는지는 설정 화면으로 확인해
> - **로그인할 때 자동 실행은 아직 없어.** 매번 더블클릭해야 해
> - **윈도우용은 맥에서 만들어서 아직 윈도우에서 돌려보지 못했어.** 공식 `node.exe` 에
>   주입하는 방식이라 원래 서명이 깨져서, SmartScreen 이 막으면 `추가 정보 → 실행` 으로 넘어가줘

### ⌨️ 길 2 — 소스로 (고칠 생각이면 이쪽)
...
Node 24 이상이 필요해 (`node -v`). 빌드 단계는 없어 — TypeScript 를 바로 읽거든.
켜면 터미널에 설정 화면 주소가 찍혀. 나머지는 길 1 의 3~6번과 같아.

터미널을 계속 띄워놔야 하는 게 싫으면 아래 **로그인할 때 자동으로 켜기** 를 봐.

## 🖥️ 설정 화면

`.env` 를 손으로 열 필요 없어. 브라우저에서 다 돼.

```
http://127.0.0.1:7979
```

- **🚀 시작하기** — 어느 대화로 갈지 정하기 (이미 정해졌으면 안 보여)
- **🧚 지금** — 상태, 다음에 갈 시각, 오늘 편 횟수 / `지금 한 번 보내보기` `폈어요`
- **⏱️ 언제 갈지** — 간격·시작·끝·요일·타임존 (저장하면 즉시 반영)
- **📮 어디로 갈지** — 토큰과 ID (바꾸면 다시 깨워야 반영돼)
...
| 하고 싶은 것 | 설정 화면에서 | 채팅으로 |
|---|---|---|
| 알림 잠깐 끄기 | `⏸ 알림 멈추기` | `/stop` 또는 `/pause` |
| 다시 켜기 | `▶ 알림 시작하기` | `/start` 또는 `/resume` |
| 나 자체를 끄기 | `🚪 나 끄기` | — |

`🚪 나 끄기` 는 곱게 끄는 길이야. 설정과 기록은 그대로 남아.
터미널로 켰다면 `Ctrl+C` 도 같고, 급하면 `pkill -f "척추의 요정"` 도 돼.

창 없이 띄웠을 땐 `🚪 나 끄기` 가 사실상 유일한 길이야. Dock 아이콘이 없거든.

## 🌙 로그인할 때 자동으로 켜기

**소스로 쓸 때만** 돼. (실행파일용 자동 시작은 아직 없어)
...
| `FAIRY_OPEN` | | `off` 면 브라우저를 안 열어. 기본은 실행파일이면 늘, 소스면 처음 쓸 때만 |

△ = 이 둘 중 최소 하나는 있어야 해. 둘 다 주면 **양쪽 모두로** 찾아갈게. 🧚🧚

갈 곳을 환경변수로 정해두면 저장해둔 값보다 그게 우선이야.
토큰은 설정 화면에서 바꾸는 게 편해. 어느 쪽으로 바꾸든 **나를 다시 깨워야** 반영돼 —
채널 연결은 시작할 때 한 번만 맺거든.
...
내 짐은 **나와 같은 폴더**에 쌓여. 소스로 돌리면 저장소 안, 실행파일로 돌리면 그 옆.
git 에는 안 올라가. 📂
